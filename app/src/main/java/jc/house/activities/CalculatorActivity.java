...
public class CalculatorActivity extends BaseActivity implements View.OnClickListener {
    private Spinner paymentTypeSpinner, firstPaymentSpinner, loanTimeSpinner, rateDiscountSpinner;
    ...
    private void initView() {
        ...
        this.rateDiscountSpinner = (Spinner) findViewById(R.id.interest_rate_discount);//年利率折扣
        ...
        ArrayAdapter<String> paymentTypeAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, paymentTypeList);
        ...
        List<String> rateDiscountList = new ArrayList<>();
        for (double i = 7; i <= 10; i = i + 0.5) {
            rateDiscountList.add(i + "");
        }
        ArrayAdapter<String> rateDiscountAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, rateDiscountList);
        rateDiscountAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        rateDiscountSpinner.setAdapter(rateDiscountAdapter);
        rateDiscountSpinner.setSelection(6);

        //首付几成
        List<String> firstPayList = new ArrayList<>();
        for (int i = 2; i <= 9; i++) {
            firstPayList.add(i + "成");
        }
        ArrayAdapter<String> firstPayAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, firstPayList);
        firstPayAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        firstPaymentSpinner.setAdapter(firstPayAdapter);

        //贷款期限
        List<String> loanTimeList = new ArrayList<>();
        for (int i = 1; i <= 5; i++) {
        ...
        ArrayAdapter<String> loanTimeAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, loanTimeList);
    ...
    private void calculate() {
        ...
        if (check(unitPrices) && check(areas) && check(interestRates)) {
            ...
            String rateDiscount = (String) rateDiscountSpinner.getSelectedItem();
            ...
            if ((paymentTypeSpinner.getSelectedItem()).equals("等额本息")) {
            ...
            } else if ((paymentTypeSpinner.getSelectedItem()).equals("等额本金")) {
